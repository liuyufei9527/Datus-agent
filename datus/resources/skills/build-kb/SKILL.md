...
2. Aggregate every explore result **together with the `reference_sql` rows you enumerated directly in Step 1**, **dedupe** refs that appear under multiple domains, and build a **Generation Manifest** grouped by store, rendered as a Markdown table:
   ...
   | … | knowledge | `status` enum on `orders` | `extract-knowledge` (deep) | atomic field-encoding fact |
...
2. **Make every delegated prompt self-contained (see storage-classify's *Context Handoff*).** The `explore` subagents that produced these refs are gone, and each generator runs in a fresh context — so inline the **datasource (+ dialect)**, the **business intent**, the **`prompt-seed` the explorer returned**, and the **rules/encodings already gathered** that the artifact must honor. Route each manifest item to its store with the prescribed mechanism:
   ...
   - **Heavy items** → delegate (the placeholders below are the *minimum* each prompt must carry):
     ...
     - knowledge → run `extract-knowledge` in **deep** mode (say "deep mode" in the invocation directive so it pins deep instead of defaulting to lite non-interactively); pass the **source (the SQL/doc/table) and the specific fact to mine**, plus the datasource it applies to. Deep drives a blind `gen_sql` subagent (the subagent never sees the gold SQL) and mines facts only from the validated diff — this prevents a main-agent schema-exploration hunch (e.g. "filter on a level column it happened to notice") from being written as a "fact" when the gold never relied on it. Only mine atoms `/init` did not already file — do not duplicate existing `./knowledge/*.md` entries.
3. **Ordering:** metrics build on semantic models — generate all `semantic_models` items **before** their dependent `metrics` items.
4. **Dual-route every `(question, SQL)` pair that appears as a `knowledge` row in the manifest — this is required, not optional.** For each such pair: (a) send it to `gen_sql_summary` so the example (with its original question) lands in `reference_sql`, AND (b) feed the same pair to `extract-knowledge` (deep) to mine the non-inferable rule. The example teaches *answer shape* (retrieved later for few-shot); the mined atom teaches *why* (encodings, mandatory filters, term→column mappings). One source, two stores — neither replaces the other. Mine knowledge ONLY for pairs listed as `knowledge` rows in the manifest — whether the user confirmed them, or auto-run carried the manifest through unconfirmed (see Turn Boundary).
...
- **Mine-only mode (no reference_sql indexing).** When the invocation directive says to mine the gold SQL for knowledge only and NOT to index reference_sql (e.g. when enumerating one summary per `(question, SQL)` pair is too slow/costly for the build), honor it strictly: enumerate **zero** `reference_sql` rows even though the corpus has a `sql` column, and feed each `(question, SQL)` pair only to `extract-knowledge` (**deep** mode — the blind `gen_sql` subagent validates each atom against the real result without ever seeing the gold SQL) to distill generalized atomic facts. Knowledge is atomic facts, not answers, so **never copy a whole SQL statement or a `question → SQL` pair into `knowledge`, `semantic_models`, or any store** — persist only the distilled facts.
